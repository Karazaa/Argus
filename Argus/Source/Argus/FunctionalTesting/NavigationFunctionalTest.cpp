...
void ANavigationFunctionalTest::StartNavigationThroughWaypointsTestStep()
{
	...
	if (taskComponent && targetingComponent && navigationComponent)
	{
		...
		navigationComponent->m_queuedWaypoints.PushLast(NavigationWaypoint(m_waypoint1));
		navigationComponent->m_queuedWaypoints.PushLast(NavigationWaypoint(m_waypoint2));
		taskComponent->m_movementState = EMovementState::ProcessMoveToLocationCommand;
	}
	else
	{
